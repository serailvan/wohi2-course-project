      ...
      try {
        ...
        if (result.correct) {
          ...
          if (scoreValues.length >= 2) {
            const currentText = scoreValues[1].textContent; 
            
            // 3. Split it to get the numbers
            const parts = currentText.split("/");
            let solved = parseInt(parts[0]);
            const total = parts[1];

            scoreValues[1].textContent = `${Math.min(solved + 1, total)}/${total}`;
          }
